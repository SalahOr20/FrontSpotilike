...
function Albums() {
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:8000/albums/')
      .then(response => {
...
`;
  ...
  const fetchGenders = () => {
    axios.get('http://127.0.0.1:8000/genders/')
      .then(response => setGenders(response.data));
  };

  const handleAlbum = (album) => {
    axios.get(`http://127.0.0.1:8000/album/${album.id_album}/`)
      .then(response => {
        setSelectedAlbum(response.data);
        return axios.get(`http://127.0.0.1:8000/album/${album.id_album}/songs`);
      })
      .then(response => {
  ...
  const handleDelete = (album) => {
   ...
   console.log(headers)
    axios.delete(`http://127.0.0.1:8000/albums/${album.id_album}/delete_album/`, { headers })
      .then(response => {
